...
void Grid::replay_move() {
	...
	if((dest == "W") || (dest == "w")){
		if(move_forward()!=-1)
			...
			cout << "You cannot enter the realm of the lost souls for whomever joins the dead enver sees light again." << endl;
			location = last_location;
			move();
		}
	}
		...
		quitGame();
		cout << "Are you sure you wwanna quit the game? ";
		string s;
		cin >> s;
		if((s == "NO") || (s == "no"))
			this->play = 0;
		...
		cout << "Not a possible move. Please choose W,S,A,D " << endl;
		move();
	}
}

void Grid::show_location(){
	if(Map[(location/10)][location%10] == 0){
		Display_Map();
		...
		if (possibility == 0) {
			...
			cout << endl << endl << endl << KRED << "\t\t\t\t" << BOLD(FYEL("**OH NO! The enemy has appeared. Get ready to fight!**")) << KWHT << endl << endl;
			Fight();
	...
	if(Map[(location/10)][location%10] == 1){
		this->Display_Map();
		cout << "Your current location on the map is " << (location / 10) << "x" << (location % 10) << " ." << endl;
		cout << "There is a Market in your area. Wanna visit?(YES/NO): " << endl;
		string answer;
		cin >> answer;
		if((answer == "YES") || (answer == "yes")){
			...
			while((answerl!="NO")&&(answer != "YES") && (answer!="no") && (answer != "yes")){
				cout << "There is a Market in your area. Wanna visit?(YES/NO) : " << endl;
				string answer;
				cin >> answer;
				...
				if((answer == "YES") || (answer == "yes")){
					...
					cout << "Not a valid option. Please answer correcly:!" << endl;
			}
		}
		get_standarBonus("hero");
		cout << "Press q to display Grid or anything else to continue" << endl;
	...
	if(Map[(location/10)][location%10] == 2){
		cout << endl << endl << KRED<< "\t\t\t\t\t\t\t W A R N I N G! " << KWHT << endl << endl;
		cout << "\t\tYou cannot enter the warlods court. Get away from the walls for they are not as friendly as them seem." << endl << endl;
		location = last_location;
		cout << "Replay your move" << end;
	}
}
			...
			if((*it)->get_healthP() > 0) {
				...
				while(flag == 0){
					...
					if((answer == "EQUIP") || (answer == "equip")){
						...
						cout << endl << "Not a valid option! Choose another answer." << endl << endl;
				}
			}
			else
...
void Grid::display_monsterStatus(){
	...
	cout << endl;
	cout << BOLD(FYEL("You Enemy's Troops:")) << endl;
...
void Grid::gameplay() {
	cout << endl;
	cout << '\t' << "Do you want to view the basic information for the game? Press yes/YES or no/NO to start the game immideately." << endl << "\t";
	string option;
	cin >> option;
	...
	cout << endl;
	cout << "\t\t\t\t\t" << BOLD(FBLU("LET THE GAME BEGIN")) << endl << endl << endl;	
	...
	for(int i = 0; i < 3; i++) {
		...
		if(i<2) {
			cout << "Do you want to choose another player? YES/NO" << endl;
			string s;
			cin >> s;
			if((s=="NO") || (s=="no")) {
				break;
				...
				cout << "Not a valid option! Please answer YES/NO" << endl;
				cin >> s;
